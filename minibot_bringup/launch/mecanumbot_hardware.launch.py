...
def generate_launch_description():
    ...
    urdf_path = os.path.join(get_package_share_directory('minibot_description'), 'urdf', 'mecanumbot.urdf')
    urdf_doc = xacro.parse(open(urdf_path, 'r'))
    xacro.process_doc(urdf_doc)
    robot_description = urdf_doc.toxml()

    robot_controller_config = os.path.join(get_package_share_directory('minibot_description'), 'config', 'robot_controller_config.yaml')
